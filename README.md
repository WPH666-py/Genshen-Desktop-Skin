...
py -3 -m genshen_skins list                   # 看全部 32 套
...
见顶部示例。AI 读取 `catalog.json`（32 套的能力矩阵）与 `AGENTS.md`（分环境安装指引），
不需要你懂任何命令。
...
| `sync --all` | 把 32 个仓库克隆到 `~/.genshen-skins` |
...
├── README.md              ← 本文件
├── catalog.json           ← 32 套皮肤机器可读目录（能力矩阵/路径/扩展ID/壁纸）

...
本仓库是 **34 套原神角色动态皮肤的集合**：机器可读目录 `catalog.json` + 跨平台安装器
...
- 不确定是哪一套 → 读 `catalog.json` 的 `skins[].char` / `name` / `aliases` 匹配；
  匹配不到就列几个候选问用户。
- **不要**在没有确认的情况下给 34 套全部换壁纸（那是 34 次系统壁纸切换）。
...
桑多涅=sandrone、克洛琳德=clorinde、茜特拉莉=citlali、诺艾尔=noelle=女仆=西风骑士团、伊涅芙=eneffa=霜蓝鎏金、瓦蕾莎=varisha=甜甜花、奥黛塔=odette=霜羽、沃雅妮莎=vodyanitsa=清露。
...
py -3 -m genshen_skins vendor --out D:\GenshenAll      # 把 34 个仓库全部落地到指定目录
```

34 套逐套跑 `install` 会反复改系统壁纸，通常没必要；「都装上」一般指克隆 + 装扩展。

...
"""genshen_skins.cli —— `genshen-skin` 命令行。
    ...
    genshen-skin vendor --out <目录>       把 29 个仓库全部落地（离线收藏）
...
def resolve(key, cat=None, strict=True):
    ...
    if cands:
        ...
        print("没找到 %r。用 `genshen-skin list` 看全部 29 套。" % key, file=sys.stderr)
    if strict:
        raise SystemExit(2)
    return None
...
def build_parser():
    ap = argparse.ArgumentParser(
        prog="genshen-skin",
        description="原神桌面皮肤集合（29 套）：壁纸 / IDE 扩展 / 桌宠 / DSH 插件 一键安装",
    ...
    p.add_argument("key", nargs="?", default=None)
    p.add_argument("--all", action="store_true", help="克隆全部 29 套")
    p.set_defaults(func=cmd_sync)

    p = sub.add_parser("vendor", help="把 29 个仓库全部落地到指定目录（离线收藏）")

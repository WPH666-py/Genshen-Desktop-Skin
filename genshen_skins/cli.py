...
"""genshen_skins.cli —— `genshen-skin` 命令行。
    ...
    genshen-skin vendor --out <目录>       把 34 个仓库全部落地（离线收藏）
...
def resolve(key, cat=None, strict=True):
    ...
    if cands:
        ...
        print("没找到 %r。用 `genshen-skin list` 看全部 34 套。" % key, file=sys.stderr)
    if strict:
        raise SystemExit(2)
    return None
...
── 镜像 / 同步 ─────────────────────────────────────────────────────
  sync --all               克隆 34 个仓库到 ~/.genshen-skins
...
def build_parser():
    ap = argparse.ArgumentParser(
        prog="genshen-skin",
        description="原神桌面皮肤集合（34 套）：壁纸 / IDE 扩展 / 桌宠 / DSH 插件 一键安装",
    ...
    sub.add_parser("list", help="列出全部 34 套皮肤").set_defaults(func=cmd_list)
    ...
    p.add_argument("key", nargs="?", default=None)
    p.add_argument("--all", action="store_true", help="克隆全部 34 套")
    p.set_defaults(func=cmd_sync)

    p = sub.add_parser("vendor", help="把 34 个仓库全部落地到指定目录（离线收藏）")
